...
class PersonalAnnotationsDAL:
    ...
    async def delete_all_personal_annotation(self, id: int, stream_group: str, day: str, parity: str):
        await self.db_session.execute(delete(PersonalAnnotation).where(PersonalAnnotation.id == id,
                                                                       PersonalAnnotation.stream_group == stream_group,
                                                                       PersonalAnnotation.day == day,
                                                                       PersonalAnnotation.parity == parity))

    async def delete_personal_annotation(self, id: int, stream_group: str, day: str, parity: str):
        await self.db_session.execute(delete(PersonalAnnotation).where(PersonalAnnotation.id == id,
                                                                       PersonalAnnotation.stream_group == stream_group,
                                                                       PersonalAnnotation.day == day,
                                                                       PersonalAnnotation.parity == parity))
