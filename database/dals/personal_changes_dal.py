...
class PersonalChangesDAL:
    ...
    async def get_all_personal_changes(self, id: int, stream_group: str, day: str, parity: str) -> List[PersonalChange]:
        q = await self.db_session.execute(select(PersonalChange.changes).where(PersonalChange.id == id,
                                                                               PersonalChange.stream_group == stream_group,
                                                                               PersonalChange.day == day,
                                                                               PersonalChange.parity == parity))
    ...
    async def get_all_personal_changes_pair_numbers(self, id: int, stream_group: str, day: str, parity: str) -> List[PersonalChange]:
        q = await self.db_session.execute(select(PersonalChange.pair_number).where(PersonalChange.id == id,
                                                                                   PersonalChange.stream_group == stream_group,
                                                                                   PersonalChange.day == day,
                                                                                   PersonalChange.parity == parity))
        return q.scalars().all()

    async def delete_all_personal_changes(self, id: int, stream_group: str, day: str, parity: str):
        await self.db_session.execute(delete(PersonalChange).where(PersonalChange.id == id,
                                                                   PersonalChange.stream_group == stream_group,
                                                                   PersonalChange.day == day,
                                                                   PersonalChange.parity == parity))

    async def delete_personal_changes(self, id: int, stream_group: str, day: str, parity: str, pair_number: int):
        await self.db_session.execute(delete(PersonalChange).where(PersonalChange.id == id,
                                                                   PersonalChange.pair_number == pair_number,
                                                                   PersonalChange.stream_group == stream_group,
                                                                   PersonalChange.day == day,
