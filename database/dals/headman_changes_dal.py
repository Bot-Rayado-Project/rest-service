...
class HeadmanChangesDAL:
    ...
    async def create_headman_changes(self, stream_group: str, day: str, parity: str, pair_number: int, changes: str) -> None:
        new_change = HeadmanChange(stream_group=stream_group, day=day,
                                   parity=parity, pair_number=pair_number, changes=changes)
    ...
    async def get_all_headman_changes(self, stream_group: str, day: str, parity: str) -> List[HeadmanChange]:
        q = await self.db_session.execute(select(HeadmanChange.changes).where(HeadmanChange.stream_group == stream_group,
                                                                              HeadmanChange.day == day,
                                                                              HeadmanChange.parity == parity))
        return q.scalars().all()

    async def get_all_headman_changes_pair_numbers(self, stream_group: str, day: str, parity: str) -> List[HeadmanChange]:
        q = await self.db_session.execute(select(HeadmanChange.pair_number).where(HeadmanChange.stream_group == stream_group,
                                                                                  HeadmanChange.day == day,
                                                                                  HeadmanChange.parity == parity))
        return q.scalars().all()
    ...
    async def delete_headman_changes(self, stream_group: str, day: str, parity: str, pair_number: int):
        await self.db_session.execute(delete(HeadmanChange).where(HeadmanChange.pair_number == pair_number,
                                                                  HeadmanChange.stream_group == stream_group,
                                                                  HeadmanChange.day == day,
                                                                  HeadmanChange.parity == parity))
