...
async def get_schedule(id: int, stream_group: str, parity: str, day: str | None = None) -> dict:
    ...
    stream_group = translit(stream_group, language_code='ru', reversed=True) if detect_language(
        stream_group) is not None else stream_group
...
async def change_schedule_headman(request: ChangeScheduleHeadman) -> dict:
    ...
    stream_group = translit(request.stream_group, language_code='ru', reversed=True) if detect_language(
        request.stream_group) is not None else request.stream_group
    day = DAYS_MATCHING.get(
        request.day) if request.day not in DAYS_ENG else request.day
    parity = PARITY_MATCHING.get(
        request.parity) if request.parity not in PARITY_RU else request.parity
